        ...
        for (let level = 1; level <= callStackLevel; level++) {
            ...
            for (const item of currentLevelItems) {
                ...
                for (const call of outgoingCalls) {
                    ...
                    // Skip if not in the workspace (library functions)
...
function extractImportTargets(fileContent) {
    ...
    // 1. Quoted imports: e.g. import ... from "./foo"; require("./bar"); #include "baz.h"; from "./foo" import ...
    const quotedRegexes = [
        /import\s+.*?\s+from\s+['"]([^'"]+)['"]/g,
        /require\(\s*['"]([^'"]+)['"]\s*\)/g,
        /#include\s+["<]([^">]+)[">]/g,
    ...
    // 2. Package/Namespace/Module imports: using Name.Space; import package.Name; import module; from module import member;
    const wordRegexes = [
        /using\s+([\w.]+)\s*;/g,
        /import\s+([\w.]+)\s*;/g,
        /from\s+([\w.]+)\s+import/g
    ...
    // Special case for Python "import module" (which can be a single word on the line)
    ...
    while ((match = pythonImportRegex.exec(fileContent)) !== null) {
        ...
        for (const p of parts) {
            const cleanPart = p.trim().split(/\s+/)[0]; // handle "import foo as bar"
...
async function resolveTargetToUris(target, currentFilePath, workspaceFolders) {
    ...
    // 1. If relative path (starts with .)
    if (target.startsWith('.')) {
        ...
        // Use vscode.workspace.findFiles with relative pattern if not found
        ...
        // 2. Absolute or package/module/namespace import
        const dottedPath = target.replace(/\./g, '/');

        // Check if there is an exact or relative path under workspace root matching dottedPath
        if (workspaceRoot) {
        ...
        // Try searching for the last segment as a file name
        ...
        if (lastSegment && lastSegment.length > 1) {
            ...
            // Fallback: executeWorkspaceSymbolProvider
            try {
...
function getSurroundingCodeContext(doc, uri, lineNumber) {
    ...
    const cursorLine = lineNumber - 1; // 0-based
...
async function getInitialTypeContext(filePath, lineNumber) {
    const line = lineNumber - 1; // 0-based
    let uri;
    if (path.isAbsolute(filePath)) {
        uri = vscode.Uri.file(filePath);
    } else {
...
async function getInitialPromptContext(filePath, lineNumber, callStackLevel = 2) {
    ...
    try {
        ...
        // ignore errors to allow fallback
    }
    
    const fileContext = await getInitialFilePathContext(absolutePath, callStackLevel);

    // If callerContext is empty, we fall back to surrounding 50 lines
    if (!callerContext) {
        try {
        ...
        calleeContext = ""; // No callee context if no enclosing symbol
    }
...
module.exports = getInitialPromptContext;
