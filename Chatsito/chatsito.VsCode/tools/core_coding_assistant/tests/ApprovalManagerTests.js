...
describe('ApprovalManager Tests', function() {
    ...
    it('should auto-approve in ai mode', async () => {
        const result = await ApprovalManager.requestApproval('run_command', { command: 'echo 1' }, 'ai');
    ...
    it('should prompt in user mode even for non-destructive tools', async () => {
        ...
        const result = await ApprovalManager.requestApproval('read_file', { file_path: 'test.js' }, 'user');
    ...
    it('should auto-approve non-destructive tools in hybrid mode', async () => {
        ...
        const result = await ApprovalManager.requestApproval('read_file', { file_path: 'test.js' }, 'hybrid');
    ...
    it('should prompt for command execution in hybrid mode', async () => {
        mockUserSelection = 'Approve';
        const result = await ApprovalManager.requestApproval('run_command', { command: 'echo 1' }, 'hybrid');
        assert.strictEqual(result, true, 'Should approve if user clicks Approve');
        
        mockUserSelection = 'Reject';
        const result2 = await ApprovalManager.requestApproval('run_command', { command: 'echo 1' }, 'hybrid');
    ...
    it('should properly detect workspace boundaries in hybrid mode', async () => {
        ...
        let result = await ApprovalManager.requestApproval('insert_code', { file_path: insidePath }, 'hybrid');
        ...
        mockUserSelection = 'Reject';
        result = await ApprovalManager.requestApproval('insert_code', { file_path: outsidePath }, 'hybrid');
    ...
    it('ResponseRouter should respect ApprovalManager result', async () => {
        ...
        const msg = {
            tool_calls: [{
                function: { name: 'run_command', arguments: { command: 'echo 1' } }
            }]
        };
