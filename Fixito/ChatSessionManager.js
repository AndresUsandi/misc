...
import getInitialPromptContext from './ContextBuilder.js';
...
class ChatSessionManager {
    ...
    loadSavedSessionMetadata() {
        if (!this.context || !this.context.workspaceState) return;
    ...
    async restoreSession(sessionId) {
        if (!this.context || !this.context.workspaceState) return false;
        const sessions = this.context.workspaceState.get('chatsito.saved_sessions') || [];
        ...
        try {
            ...
            if (this.conversationHistory.length === 0) {
                this.conversationHistory.push({
                    role: 'system',
                    content: `You are Fixito, a code review assistant.

You receive code reviews and validate them by making sure they are as high quality changes as they could possibly be.
