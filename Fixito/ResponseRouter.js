...
class ResponseRouter {
    ...
    static async routeToolCall(toolCall, webviewClient, toolManager, selectedMode) {
        if (toolCall && toolCall.function) {
            ...
            if (webviewClient && webviewClient.sendProgress) {
                webviewClient.sendProgress(progressText);
            }
            logger.log(`[ResponseRouter] Routing tool ${toolName}`);
            ...
            let resultString = typeof toolResult === 'string' ? toolResult : (logger.formatJson ? logger.formatJson(toolResult) : JSON.stringify(toolResult));
